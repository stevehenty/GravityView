 ...
 *
 * @return false|int|int[] ID (or array of IDs) of the forms connected to the View, if they exist. False if no connected the View ID isn't valid.
 */
function gravityview_get_form_id( $view_id ) {
	...
	/**
	 * @filter `gravityview/view/{View ID}/get_form_id` Modify the form ID for a View.
	 * @since 1.8
	 ...
	 */
	$id = apply_filters( 'gravityview/view/' . $view_id . '/get_form_id', $id, $view_id );

	/**
	 * @filter `gravityview/view/get_form_id` Modify the form ID for a View.
	 * @since 1.8
	 * @param int|boolean $id The ID of the connected form, if exists. Otherwise, false.
	 * @param int $view_id The ID of the View we're fetching the form for
	 */
	$ids = apply_filters( 'gravityview/view/get_form_id', $id, $id );
